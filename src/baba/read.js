export const read = tk => {
    const fatal = m => { throw `(${tk.pos()[0]}:${tk.pos()[1]}:${tk.pos()[2]}): ${m}`; }, list = (e, cont, end) => {
        const opers = [];
        if (tk.take(end)) {
            return opers;
        }
        ...
        if (cl) {
            return { kind: "lit", value: cl[1] === "null" ? null :
    ...
    }, postfix_rhs = lhs => tk.take("lparen") ? postfix_rhs(call(lhs) || fatal("Expected an expression.")) :
    ...
    }, prefix = () => tk.take("plus") ? { kind: "pos", oper: prefix() || fatal("Expected an expression.") } :
        ...
        if (tk.take("fun")) {
            ...
            function go() {
                ...
                if (tk.take("if"))
                    return { kind: "ifelse", cond, then, else: go() };
                if (!tk.take("lbrace"))
                    fatal("Expected `{`.");
