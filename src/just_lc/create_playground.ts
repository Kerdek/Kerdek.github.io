...
export function create_playground(initial: string): [HTMLElement, IStandaloneCodeEditor] {
  ...
  async function ev() {
    ...
    try {
      ...
      const n: Graph = evaluate((_rec, rc, _ret) => rc({ kind: "app", lhs: { kind: "app", lhs: f, rhs: { kind: "lit", val: x => x.kind === "lit" && typeof x.val === "number" ? { kind: "lit", val: x.val + 1 } : { kind: "lit", val: 0 } } }, rhs: { kind: "lit", val: 0 } }))
