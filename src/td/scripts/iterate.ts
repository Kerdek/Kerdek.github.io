...
export const enumerate = <K extends string | number | symbol, E, KP extends string | number | symbol, EP>(l: { [i in K]: E }, f: (k: K, x: E) => [KP, EP]) => {
...
export const async_enumerate = async <K extends string | number | symbol, E, KP extends string | number | symbol, EP>(l: { [i in K]: E }, f: (k: K, x: E) => Promise<[KP, EP]>) => {
  const r: { [i in KP]: EP } = {} as any
  for (const k of Reflect.ownKeys(l)) {
...
export const async_map = async <E, F>(l: E[], f: (x: E) => Promise<F>) => {
  const r: F[] = []
  for (const x of l) {
    r.push(await f(x)) }
  return r }
