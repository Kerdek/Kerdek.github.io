...
export const add_script_editor = (text?: string | undefined, name?: string): void => {
const
input = mod(elm('div'), e => {
  assign(e.style, {
    userSelect: 'none',
...
open = () => {
  const
    item = (label: string, handler: () => void) =>
      mod(list_item(label), e => {
    ...
    list = mod(elm('div'), e => {
      e.append(
        item('..', put_back),
        ...keys(files).sort().map(name => item(name, () => {
