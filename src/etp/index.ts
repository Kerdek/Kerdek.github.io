...
import { Context, empty_context } from "./lang.js"
  ...
  left_pane = await ela("div", async e => {
    const
      ...
      check = async () => {
        ...
        await refresh()
        const [a, m] = read_article_abstract(
          tokenizer(s, { line: 1, col: 1 }))
        const pfx =
          chapters.slice(0,
            chapters.indexOf(active_chapter))
          .reduce((
            { sigma, rho, pi },
            { exports: { sigma: sigmap, rho: rhop, pi: pip }}) => ({
              sigma: [...sigma, ...sigmap],
              rho: [...rho, ...rhop],
              pi: [...pi, ...pip] }), empty_context())
    ...
    await reset(
      localStorage.getItem('proof-playground') ||
    ...
    e.append(
      elm("div", e => {
        assign(e.style, {
          ...
          flexDirection: "row" })
        let b = {}
        const button = (msg: string, title: string, f: () => void) => elm('div', e => {
          assign(e.style, {
            ...b,
            textAlign: "center",
            cursor: "pointer",
