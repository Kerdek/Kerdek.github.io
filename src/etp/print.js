...
import { visit_proposition } from "./lang.js";
import { query, reduce } from "./check.js";
const { assign } = Object;
const highlight = (c) => (s) => elm('span', e => {
    assign(e.style, {
        color: c
    });
    e.append(txt(s));
}), pp = highlight(colors.proposition), pf = highlight(colors.proof), pps = highlight(colors.propositionsymbol), pfs = highlight(colors.proofsymbol), hparens = (c, s) => !c ? s : [pps(`(`), ...s, pps(`)`)], proposition = run(({ proc, call, cc, ret }) => {
    const lambda = proc(({ b }) => b.k === 'lam' ?
        call(lambda(b), db => ret([txt(` `), pp(b.i), ...db])) :
        call(main(b, { p: 0, t: 0 }), dx => ret([pps(`.`), ...dx])));
    const main = proc(visit_proposition({
        lam: ({ i, ...r }, { t }) => di(t > 0, s => call(lambda({ i, ...r }), du => ret(hparens(s, [
            pps(`\\`), pp(i), ...du
        ])))),
        imp: ({ l, r }, { p, t }) => di(p > 0, s => call(main(l, { p: 1, t: 1 }), dl => call(main(r, { p: 0, t: s ? 0 : t }), dr => ret(hparens(s, [
            ...dl, txt(` `), pps(`->`), txt(` `), ...dr
        ]))))),
        app: ({ l, r }, { p, t }) => di(p > 1, s => call(main(l, { p: 1, t: 2 }), dl => call(main(r, { p: 2, t: s ? 0 : t }), dr => ret(hparens(s, [
            ...dl, txt(` `), ...dr
        ]))))),
        ref: ({ i }, {}) => ret([pp(i)]),
...
}), pos = (w) => `(${w.line}, ${w.col})`, proof_name = (c) => [
    pfs(`<`),
    pf(c),
    pfs(`>`)
], hover = (e, h) => {
...
}, goal = (pfx, { tau, sigma, rho, pi }) => [
    ...pi.length === 0 ? [] : [
        elm('div', e => {
            e.append(...pi.map(i => [pp(i), txt(` `)]).flat(1));
        })
    ],
    ...rho.map(({ i, d }) => elm('div', e => {
        hover(e, proposition(query(d, [...pfx.rho, ...rho])));
        e.append(pp(i), txt(` `), pfs(`:=`), txt(` `), ...proposition(reduce(d, [...pfx.rho, ...rho])));
    })),
    ...sigma.map(({ i, t }) => elm('div', e => {
        hover(e, proposition(query(t, [...pfx.rho, ...rho])));
        e.append(...proof_name(i), txt(` `), pfs(`:`), txt(` `), ...proposition(reduce(t, [...pfx.rho, ...rho])));
    })),
    elm('div', e => {
        hover(e, proposition(query(tau, [...pfx.rho, ...rho])));
        e.append(pfs(`⊢`), txt(` `), ...proposition(reduce(tau, [...pfx.rho, ...rho])));
    })
];
export const print_messages = (pfx, g, curse) => g.map(({ w, m, c }) => elm('div', e => {
    assign(e.style, {
        margin: "3pt",
        padding: "3pt",
        borderColor: colors.guide,
...
export const print_proposition = run(({ proc, call, cc, ret }) => {
    const lambda = proc(({ b }) => b.k === 'lam' ?
        call(lambda(b), db => ret(` ${b.i}${db}`)) :
        call(main(b, { p: 0, t: 0 }), dx => ret(`.${dx}`)));
    const main = proc(visit_proposition({
        lam: ({ i, ...r }, { t }) => di(t > 0, s => call(lambda({ i, ...r }), du => ret(parens(s, `\\${i}${du}`)))),
        imp: ({ l, r }, { p, t }) => di(p > 0, s => call(main(l, { p: 1, t: 1 }), dl => call(main(r, { p: 0, t: s ? 0 : t }), dr => ret(parens(s, `${dl} -> ${dr}`))))),
        app: ({ l, r }, { p, t }) => di(p > 1, s => call(main(l, { p: 1, t: 2 }), dl => call(main(r, { p: 2, t: s ? 0 : t }), dr => ret(parens(s, `${dl} ${dr}`))))),
        ref: ({ i }, {}) => ret(i),
        var: ({ d }, ctx) => d[0] ? cc(main(d[0], ctx)) :
            ret(`()`),
        err: ({}, {}) => ret(`()`)
