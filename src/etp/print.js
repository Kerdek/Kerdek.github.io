...
export const print_prop = (e, b) => homproc((call, _cc, ret) => {
    const p = e => () => e.body.kind === "all" ? (x => call(p(x), dx => ret(` ${x.schema ? "*" : ""}${x.id}${dx}`)))(e.body) : call(s(e.body)(0, true), dx => ret(`.${dx}`));
    const t = visit({
        all: e => (_p, r) => call(p(e), dx => ret(parens(!r, `∀${e.schema ? "*" : ""}${e.id}${dx}`))),
