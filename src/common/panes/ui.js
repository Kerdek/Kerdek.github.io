...
export const separator = () => mod(elm('div'), e => {
    assign(e.style, {
        height: '1px',
...
}), simple_tool_tip = (e, tip) => tool_tip(e, () => ({ items: [{ type: 'element', mod: e => { e.append(txt(tip)); } }] })), tool_tip = (e, body) => {
    let my_end;
    e.addEventListener('pointerenter', ev => {
        const refresh = (x, y, cx, cy) => {
            if (on_move) {
                on_move(cx, cy);
            }
        ...
        const { items, on_move, dispose } = body(), tip = mod(elm('div'), e => {
            assign(e.style, {
                paddingLeft: '8px',
                paddingRight: '8px',
                zIndex: '100001',
            ...
            e.append(mod(elm('div'), e => {
                ...
                e.append(...items.map(item => item.type === 'element' ? mod(elm('div'), item.mod) :
                    separator()));
            }));
        });
    ...
    return [node, mod(elm('div'), e => {
            ...
            e.append(node, elm('br'));
        })];
}, menu_bar_item = ({ label, tip, items }) => mod(elm('div'), e => {
    assign(e.style, {
        ...right_border,
        whiteSpace: 'pre',
        display: 'inline-flex',
...
}), status_bar_item = (text) => mod(elm('div'), e => {
    assign(e.style, {
        background: colors.background,
        ...right_border,
        userSelect: 'none',
    ...
    e.append(text);
}), status_bar = (items) => mod(elm('div'), e => {
    assign(e.style, {
        background: colors.background,
        height: '18px',
        overflow: 'hidden',
    ...
    e.append(...items.map(item => mod(status_bar_item(item.label), item.mod)));
}), drop_down_menu = (x, y, pointer, items) => {
    const elems = [];
...
}, context_menu = (e, items) => {
    e.addEventListener('contextmenu', ev => {
        ev.stopPropagation();
        ev.preventDefault();
        drop_down_menu(ev.clientX, ev.clientY, ev.pointerId, items(ev.offsetX, ev.offsetY));
        return false;
    });
}, full_border = {
    borderColor: colors.foreground,
