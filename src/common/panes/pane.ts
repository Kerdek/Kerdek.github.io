...
export type CreatePane = (element: HTMLElement, options?: CreatePaneOptions) => Pane
...
activate = (pane: PaneMeta) => {
to_front(pane)
set_emph() }

let pane_spawn = 0

export const create_pane: CreatePane = (element, options): Pane => {
  ...
  let size: [number, number] = options && 'size' in options ? options.size : [300, 400]
  ...
  const title_text_box = mod(elm('div'), e => {
    ...
    e.addEventListener('pointerdown', ev => {
      if (ev.button === 0) {
        ev.preventDefault()
        ev.stopPropagation()
        const drag_total: [number, number] = [0, 0]
  ...
  const all = mod(elm('div'), e => {
    ...
    e.append(
      title_bar,
      contents_box) })
  const meta: PaneMeta = { all, index: metas.length, title_bar, close }
  metas.push(meta)
