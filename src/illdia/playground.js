...
export const create_textbox = (text, mod) => {
...
const prompt = async (title, suggest) => new Promise(c => {
    let value = undefined;
    const ok = () => {
        value = node.data;
        pane.close();
...
const alert = async (title, text) => new Promise(c => {
...
const yes_no = async (title, text) => new Promise(c => {
    let value = false;
...
const menu_bar = (items) => {
    const make_elem = (item) => create_button(item.label, function () {
        this.style.margin = '1px';
...
const run_script = (text, print) => {
    const go = async (text) => {
        const include = async (name) => {
            ...
            window['print_proxy'] = print;
            window['include_proxy'] = include;
...
const valid_filename = (name) => /^[^\0\/]+$/.test(name) && name !== '.' && name !== '..';
const prompt_filename = async (name) => {
...
const add_files_changed_listener = (handler) => {
    files_changed_listeners.add(handler);
};
const remove_files_changed_listener = (handler) => {
...
const add_file_list = () => {
    ...
    const update = () => {
        const item = (label) => create_button(label, function () {
        ...
        const list = html_element('div', function () { }, Object.keys(files).sort().map(item));
        content.innerHTML = '';
...
const add_script_editor = (text, name) => {
            ...
            items: [{
                ...
                },
                {
                    ...
                    handler: () => {
                        ...
                        const item = (label, handler) => create_button(label, function () {
                            this.style.border = 'none';
                            this.style.justifyContent = 'start';
