...
export const read = x => homproc((call, cc, ret) => {
    ...
    }, id = k(/^[^\s\\λ\.\(\)->\*]+/), ws = k(/^\s*/), ar = k(/^->/), lm = k(/^[\\λ]/), dt = k(/^\./), lp = k(/^\(/), rp = k(/^\)/), fatal = m => { throw new Error(`(${w}): ${m}`); }, parameters = () => (ws(), dt() ? cc(expression) : (param => param ? call(parameters, body => ret(abs(param, body))) : fatal("Expected `.` or an identifier."))(id())), primary = () => (ws(),
        lm() ? () => cc(parameters) :
