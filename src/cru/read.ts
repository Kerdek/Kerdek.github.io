...
import { Graph, ListSyntax, make, Mod, Module, RecordSyntax, Shr } from "./cru.js"
import { delimit } from "./evaluate.js"
import { Tokenizer, TokenKind } from "./tokenizer.js"

const includes: { [i: string]: Shr } = {}

export const read: (tk: Tokenizer) => Promise<Graph> = tk => async_homproc((call, cc, ret) => {
type Branch = ReturnType<typeof ret>
const
fatal: (m: string) => never = m => { throw new Error(`(${tk.pos()[0]}:${tk.pos()[1]}:${tk.pos()[2]}): parser: ${m}`) },
include: () => Promise<Branch> = async () => {
  ...
  return call(expression, async e => {
    tk.take("rparen")
    tk.unpos(wp)
    const m: Shr = make("shr", delimit(e)[0])
    includes[r] = m
    return ret(m) }) },
