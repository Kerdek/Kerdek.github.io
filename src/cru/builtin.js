...
export const get_builtin = await (async () => {
    ...
    const binary = op => x => (_rec, _cc, ret) => ret(y => (rec, _cc, ret) => rec(x, dx => rec(y, dy => ret(op(dx, dy)))));
    const ternary = op => x => (_rec, _cc, ret) => ret(y => (_rec, _cc, ret) => ret(z => (rec, _cc, ret) => rec(x, dx => rec(y, dy => rec(z, dz => ret(op(dx, dy, dz)))))));
    return {
