import { abs, blt, iov, term_blt, term_iov, term_lit, rec } from "./graph.js";
import { evaluate_root } from "./evaluate.js";
import { jmp } from "./run.js";
export const builtins = (() => {
    const nullary = op => term_lit(op);
    const unary = op => term_blt(r => (call, ret) => () => call(evaluate_root(r)(call, ret), dx => ret(nullary(op(dx.value)))));
    const binary = op => term_blt(r => (call, ret) => () => call(evaluate_root(r)(call, ret), dx => ret(unary(y => op(dx.value, y)))));
    const ternary = op => term_blt(r => (call, ret) => () => call(evaluate_root(r)(call, ret), dx => ret(binary((x, y) => op(dx.value, x, y)))));
    return {
        __builtin_return: term_blt(a => (_call, ret) => () => ret(term_iov("return", [a]))),
        __builtin_bind: term_blt(a => (_call, ret) => () => ret(term_blt(b => (_call, ret) => () => ret(term_iov("bind", [a, b]))))),
        __builtin_puts: term_blt(a => (_call, ret) => () => ret(term_iov("puts", [a]))),
        __builtin_new: term_blt(a => (_call, ret) => () => ret(term_iov("new", [a]))),
        __builtin_get: term_blt(a => (_call, ret) => () => ret(term_iov("get", [a]))),
        __builtin_set: term_blt(a => (_call, ret) => () => ret(term_blt(b => (_call, ret) => () => ret(term_iov("set", [a, b]))))),
        __builtin_if: term_blt(a => (_call, ret) => () => ret(term_blt(b => (_call, ret) => () => ret(term_blt(c => (call, _ret) => () => call(evaluate_root(a)(call, ret), da => jmp(evaluate_root(da.value ? b : c)(call, ret)))))))),
        __builtin_typeof: term_blt(r => (call, ret) => () => call(evaluate_root(r)(call, ret), dx => ret(term_lit(dx.kind === abs || dx.kind === blt ? "function" :
            dx.kind === iov ? "io" :
                dx.kind === rec ? "record" :
                    typeof dx.value)))),
