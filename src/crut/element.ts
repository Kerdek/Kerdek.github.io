...
export const element: (t: TypeTree, a: TypeTree) => [TypeTree, Inequalities] | string = (t, a) => {
const r: [TypeTree, Inequalities] | string = homproc((call, ret) => {
const n: (k: string[], m: TypeMap, r: [TypeTree, Inequalities] | string) => Process = (k, m, r) => () =>
  !k[0] ? ret(r) :
  di(k[0], k0 =>
  di(m[k0] as TypeTree, mk0 =>
  typeof mk0 === "string" ? ret(mk0) :
  di(subsume(typ_lit(k0), a), z =>
  typeof z === "string" ?
    jmp(n(k.slice(1), m, typeof r === "string" ? r + z : r)) :
  typeof r === "string" ?
    jmp(n(k.slice(1), m, [mk0, z])) :
  di(conjoin_inequalities(z, r[1]), c =>
  jmp(n(k.slice(1), m, [typ_dsj(r[0], mk0), c]))))))
const p: (i: number, v: string, r: [TypeTree, Inequalities] | string) => Process = (i, v, r) => () =>
  !v[0] ? ret(r) :
  di(v[0], t0 =>
  di(subsume(typ_lit(i), a), z =>
  typeof z === "string" ?
    jmp(p(i + 1, v.slice(1), typeof r === "string" ? r + z : r)) :
  typeof r === "string" ?
    jmp(p(i + 1, v.slice(1), [typ_lit(t0), z])) :
  di(conjoin_inequalities(z, r[1]), c =>
  typeof c === "string" ? ret(c) :
  jmp(p(i + 1, v.slice(1), [typ_dsj(r[0], typ_lit(t0)), c])))))
const s: (t: TypeTree, a: TypeTree) => Process = (t, a) => () =>
  a.kind === dsj ?
    call(s(t, a.lhs), dx =>
